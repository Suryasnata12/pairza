...
export function ChatPanel({ session }: { session: SessionDetail }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto overflow-x-hidden rounded-xl border border-border-subtle bg-void-elevated-2/30 p-3">
        {messages.length === 0 && (
        ...
        {messages.map((msg) => {
          if (msg.type === "system" || msg.type === "partner_joined") {
            return (
              <div key={msg.id} className="break-words py-1 text-center text-xs text-ink-faint">
                {msg.content}
              </div>
            );
          }
          if (msg.type === "discovery") {
            return (
              <div key={msg.id} className="flex items-center justify-center gap-1.5 py-1 text-center text-xs font-medium text-signal-teal">
                <Sparkles className="h-3 w-3 shrink-0" /> <span className="break-words">{msg.content}</span>
              </div>
            );
          }
          const isMe = msg.sender_id === me?.id;
          return (
            <div key={msg.id} className={cn("flex min-w-0", isMe ? "justify-end" : "justify-start")}>
              <div
                className={cn(
